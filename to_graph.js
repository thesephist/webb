...
for(i=0;i<pop_size;i++){
    for(j=0;j<pop_size;j++){
        ...
        if(writtenlines.indexOf("0x"+node_coords[j]+"x0"+node_coords[i])==-1){
            SVGlines.push("<line x1='" + node_coords[i][0] + "' y1='" + node_coords[i][1] + "' x2='" + node_coords[j][0] + "' y2='" + node_coords[j][1] + "' style='stroke:#000;stroke-width:6;opacity:" + (0.9 * ((links[i][j] - minimum) / (maximum - minimum)) + 0.1).toString() + "'/>");
        }
    }
}
